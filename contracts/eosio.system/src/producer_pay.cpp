      ...
      if( usecs_since_last_fill > 0 && _gstate.last_pervote_bucket_fill > time_point() ) {
         ...
         if( new_tokens > 0 ) {
            ...
            }
            {
               ...
               if( to_savings > 0 ) {
                  transfer_act.send( get_self(), saving_account, asset(to_savings, core_symbol()), "fund per-saving bucket" );
                  channel_to_rex( saving_account, asset(to_savings, core_symbol()) );
               }
